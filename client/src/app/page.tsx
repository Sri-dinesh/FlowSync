    ...
    <div className="flex min-h-screen flex-col bg-background text-foreground font-sans selection:bg-primary selection:text-primary-foreground">
      ...
      <main className="flex-1">
        ...
        <section className="relative flex min-h-screen flex-col items-center justify-center px-6 pt-16 text-center lg:px-8">
          ...
          <div className="relative z-10 max-w-4xl space-y-8">
            <div className="flex justify-center">
              <Badge variant="outline" className="rounded-full border-border bg-background px-3 py-1 text-xs font-medium text-muted-foreground">
                Reinforcement Learning  ·  Real-time Simulation
              </Badge>
            </div>
            
            <h1 className="text-5xl font-semibold tracking-tight sm:text-7xl">
              Smart Traffic,<br />Learned by AI
            </h1>
            ...
            <div className="flex flex-col items-center justify-center gap-4 sm:flex-row">
              <Button size="lg" className="h-12 px-8 text-base" onClick={() => router.push("/simulation")}>
                Launch Simulation
            ...
            <div className="flex flex-wrap justify-center gap-3 pt-4">
              {["DQN Agent", "WebSocket Stream", "Live Three.js Viz"].map((pill) => (
                <Badge key={pill} variant="outline" className="rounded-full border-border bg-background px-3 py-1 text-xs font-normal text-muted-foreground">
                  {pill}
                </Badge>
              ))}
            </div>
        ...
        <section className="border-t border-border bg-muted/30 px-6 py-24 lg:px-8">
          <div className="mx-auto max-w-5xl">
            <h2 className="text-center text-2xl font-semibold tracking-tight sm:text-3xl">Everything you need</h2>
            ...
            <div className="mt-16 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                <Card key={i} className="border-border bg-card shadow-none transition-colors hover:bg-muted/50">
        ...
        <section className="border-t border-border px-6 py-24 lg:px-8">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="mt-4 text-muted-foreground">Modern stack, clean architecture</p>
            
            <div className="mt-12 space-y-4">
              <div className="flex flex-wrap justify-center gap-3">
                {["Next.js", "React", "TypeScript", "Three.js", "Tailwind CSS", "Shadcn/ui"].map((tech) => (
                  <div key={tech} className="rounded-full border border-border bg-background px-4 py-1.5 text-sm text-muted-foreground">
                    {tech}
                  </div>
                ))}
              </div>
              <div className="flex flex-wrap justify-center gap-3">
                {["FastAPI", "PyTorch", "DQN", "Gymnasium", "WebSockets", "Supabase"].map((tech) => (
                  <div key={tech} className="rounded-full border border-border bg-background px-4 py-1.5 text-sm text-muted-foreground">
                    {tech}
                  </div>
                ))}
        ...
        <section className="border-t border-border px-6 py-24 lg:px-8">
          <div className="mx-auto max-w-3xl overflow-hidden rounded-2xl bg-foreground text-background">
            <div className="px-8 py-16 text-center sm:px-16">
              <h2 className="text-3xl font-semibold tracking-tight sm:text-4xl">See the AI learn.</h2>
